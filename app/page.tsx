...
export default function Home(){
 ...
 return <main>
  ...
  {tool&&<div className="modalback" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setTool(null)}}><section className="toolmodal" role="dialog" aria-modal="true" aria-label="评估工具详情"><button className="close" onClick={()=>setTool(null)}>×</button>{tool!=="10MWT"?<div className="coming"><span className="kicker">CONTENT IN PROGRESS</span><h2>{assessments.find(a=>a.abbr===tool)?.name}</h2><p>量表条目、计分规则及授权信息正在整理。完成审核后开放使用。</p><button onClick={()=>setTool(null)}>返回工具库</button></div>:<>
