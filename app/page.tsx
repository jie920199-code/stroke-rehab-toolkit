...
export default function Home() {
  ...
  type ViconFileSummary={name:string;rows:number;columns:number;left:boolean;right:boolean;events:boolean;trialType:"dynamic"|"static-or-calibration";headers:string[]};
  const [viconSummary,setViconSummary]=useState<ViconFileSummary[]>([]);
  ...
  async function importVicon(files:FileList){const summaries=await Promise.all(Array.from(files).map(async(file)=>{const text=await file.text(),lines=text.split(/\r?\n/).filter(Boolean),headers=(lines[0]||"").split(/[,;\t]/).map(x=>x.trim()).filter(Boolean),joined=`${file.name}\n${text}`.toLowerCase();return {name:file.name,rows:Math.max(0,lines.length-1),columns:headers.length,left:/left|\bl\b|左/.test(joined),right:/right|\br\b|右/.test(joined),events:/foot strike|foot off|event|触地|离地/.test(joined),trialType:/static|calib|calibration|静态|标定/.test(file.name.toLowerCase())?"static-or-calibration":"dynamic",headers:headers.slice(0,8)} as ViconFileSummary}));setViconSummary(summaries)}
          ...
          >
            ...
            {tool === "GAIT-HUB" ? (
              <>
                ...
                <section className="gaitPanel"><h3>3 · 导入Vicon Nexus CSV（支持多文件）</h3><label className="viconDrop"><input type="file" multiple accept=".csv,.txt,.tsv" onChange={e=>e.target.files?.length&&importVicon(e.target.files)} /><b>选择一组脱敏CSV文件</b><span>可同时选择时空参数、运动学、动力学及事件文件；系统按文件名和字段组合核对</span></label>{viconSummary.length>0?<div className="viconQuality"><b>已导入 {viconSummary.length} 个文件</b><p>{viconSummary.some(x=>x.left)&&viconSummary.some(x=>x.right)?"✓ 组合中检出左右侧信息":"⚠ 左右侧信息不完整，请用文件名或字段明确标识"} · {viconSummary.some(x=>x.events)?"✓ 组合中检出步态事件，可进一步核对周期":"⚠ 所有文件均未检出Foot Strike/Foot Off，不能声称完成步态周期归一化"}</p><div className="viconFiles">{viconSummary.map(file=><article key={file.name}><b>{file.name}</b><span>{file.rows}行 · {file.columns}列 · {file.trialType==="dynamic"?"动态试次":"疑似静态/标定"}</span><small>{file.left?"左 ":""}{file.right?"右 ":""}{file.events?"含事件":"无事件"} · 字段：{file.headers.join(" · ")||"无"}</small></article>)}</div></div>:null}<div className="privacy"><b>组合规则与数据质量边界</b><p>每个文件均保留来源，不按上传顺序推断左右侧；只有事件、采样范围和试次身份一致时才组合步态周期。仅在足与测力台分配明确时解释单侧动力学；缺少体重或单位时不做归一化。</p></div></section>
